...
defmodule MachineManager.CLI do
	...
	def main(argv) do
		...
		spec = Optimus.new!(
			...
			subcommands: [
				...
				connect: [
					name:  "connect",
					about: "Add connection for machines",
					args: [
						hostname_regexp: [required: true, help: hostname_regexp_help],
						connection:      [required: false, help: "Connections to add", value_name: "TAG..."],
					],
					allow_unknown_args: true,
				],
				disconnect: [
					name:  "disconnect",
					about: "Remove connection from machines",
					args: [
						hostname_regexp: [required: true, help: hostname_regexp_help],
						connection:      [required: false, help: "Connections to remove", value_name: "TAG..."],
					],
					allow_unknown_args: true,
				],
				get_tags: [
		...
		case subcommand do
			...
			:connect          -> Core.connect_many(Core.machines_matching_regexp(args.hostname_regexp),    all_arguments(args.connection, unknown))
			:disconnect       -> Core.disconnect_many(Core.machines_matching_regexp(args.hostname_regexp), all_arguments(args.connection, unknown))
	...
	defp get_column_spec() do
		%{
			...
			"connections"      => {"CONNECTIONS",      &format_connections/2},
	...
	defp format_connections(row, _) do
		row.connections
		|> Enum.map(&colorize/1)
		|> Enum.join(" ")
	end
